...
class GraphicOverlay(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    var times = SimpleDialogFragment.timesScore(context)
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        if(xy_Lists.isNotEmpty()){
            ...
            val numList = listOf<Int>(0,1, 0,2, 1,3, 2,3, 1,5, 0,4, 3,7, 2,6, 6,10, 7,11, 4,8, 5,9)
            for (i in numList){
    ...
    fun setParts(pose: Pose){
        ...
        val listPoseLandmark = mutableListOf<PoseLandmark>(
            ...
            ,pose.getPoseLandmark(PoseLandmark.RIGHT_ELBOW)!!//5
            ...
            //,pose.getPoseLandmark(PoseLandmark.LEFT_EAR)!!//12
            //,pose.getPoseLandmark(PoseLandmark.RIGHT_EAR)!!//13
            //,pose.getPoseLandmark(PoseLandmark.NOSE)!!//14
        )
    ...
    //　　　　　　前回の値　－　今回の値　＝　運動量--------------------------------------------------
    fun ExerciseIntensity(size:Size):Int{
        //初回　値を保存しておくだけ
        if(saved_xy_lists.isEmpty()){
            ...
            for(i in 0..saved_xy_lists.size - 1){
                if (xy_Lists[i].second < 0 || size.height < xy_Lists[i].second){
                    //Log.e("graphic", "パーツ画面外たて")
                    totalPoint = totalPoint + 100
                    break
                }
                if (xy_Lists[i].first < 0 || size.width < xy_Lists[i].first){
                    //Log.e("graphic", "パーツ画面外よこ")
                    totalPoint = totalPoint + 100
                    break
                }
                ...
                totalPoint = totalPoint + a * times
                //totalPoint = totalPoint * times
            }
            saved_xy_lists = xy_Lists
        }
        return totalPoint.toInt() /100
    }
